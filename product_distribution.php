...
<div id="zcontentwrapper" class="zcontentwrapper">
  <div class="pageheader notab">
    <h1 class="pagetitle">Product Distribution</h1>
  </div>
</div>
      ...
      <table style="width: 100%">
        <tr>
          ...
          <td colspan="3"> :
            ...
            to
            <input id="end_date" value="<?php echo date('Y-m-d') ?>" type="text" name="end_date" readonly/>
          </td>
        </tr>
        <tr>
          <th>Region'</th>
                ...
                <!--
                      <div style="padding: 0 10px;">
                        <span>
                          ...
                          to
                          <input id="end_date" value="<?php echo date('Y-m-d') ?>" type="text" name="end_date" class="width100" readonly/>
                        </span>
                      </div>
                      <div style="padding: 10px 10px;">
                        <span style="">
...
<!--<script type="text/javascript" src="http://maps.googleapis.com/maps/api/js?key=AIzaSyC0JhNTyjMpdmUPMDYWIVuE57dUIHZH1SU&sensor=true&libraries=places" async defer></script>-->
...
<?php echo "showMatcodeList()"; ?>
                    ...
                    $('#expcsv').click(function(e) {
                      //var str = $('#additional-parameter-symbol-combo').val();
                      //var res = str.replace("=", "#");
                      //console.log("#additional-parameter-combo : "+$('#additional-parameter-combo').val());
                      var res = 0;
                      ...
                      window.location = "<?php echo site_url('gis/export_product_distribution_csv'); ?>/" + $('#start_date').val() + "/" + $('#end_date').val() + "/" + $('#region-combo').val()
                  ...
                  function initMap() {
                    ...
                    $.ajax({
                      type: 'post',
                      url: '<?php echo base_url('gis/get_nasional_coordinat'); ?>',
                      data: $('#gis').serialize(),
                      dataType: 'json',
                      beforeSend: function() {
                        $('#loader-show').fadeIn('fast');
                        $('#outlet-counter').html('0');
                      },
...
//                          map.fitBounds(bounds);
                    ...
                    $.ajax({
                      ...
                      success: function(data) {
                        if (data.outlets.length > 0 || data.coordinats.length > 0) {
                          ...
                          var outletNow = 0;
                          var outletSource = []; // insert outlet to this var for autocomplete
                          $.each(data.outlets, function(index, outlet) {
                            if (outletNow != outlet.partner_id) {
                              ...
                              // autocomplete data
                              outlet.id = outlet.partner_id;
                              outlet.value = outlet.partner_code + ' | ' + outlet.partner_name;
                              outlet.label = outlet.partner_code + ' | ' + outlet.partner_name;
                              outletSource.push(outlet);
                          ...
                          // Create autocomplete
                          //$("#outlet-input").autocomplete('option', 'source', outletSource);
                          $("#outlet-input").autocomplete('option', {source: function(request, response) {
                              var results = $.ui.autocomplete.filter(outletSource, request.term);
                          ...
                          google.maps.event.addListener(searchBox, 'places_changed', function() {
                            ...
                            $.each(data.outlets, function(index, outlet) {
                              if (outletNow != outlet.partner_id) {
                                ...
                                //if(input.value == outlet.address){
                                if (input.value == outlet.partner_code) {
                                  map.setZoom(13);      // This will trigger a zoom_changed on the map
                                  map.setCenter(new google.maps.LatLng(outlet.lat, outlet.lon));
                            ...
                            var content = '<div id="content" style="min-width:200px"><b>' + outlet.partner_name + ' (' + outlet.partner_code + ' | ' + outlet.caption + ')</b><br>' + outlet.address;
                            if (outlet.partner_id != 0) {
                              if (outlet.partner_id != null)
                                content += '<hr>Item : <br><table width="100%">' + outlet.partner_id + '</table>';
                              else
                                content += '<hr>Tidak ada item.';
                            }
                  ...
                  /**
                   * Create Autocomplete for outlet only visible in map
